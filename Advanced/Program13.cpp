...
void finales(vector<Competidor>& v) {
	for(int i=0;i<v.size();i++) { 
		...
		cout << "\nCompetidor 1:  " << v[i].tiempo_final.segundos << endl;
		
		if (v[i].tiempo_final.segundos >= 60) { 
			v[i].tiempo_final.minutos += (v[i].tiempo_final.segundos / 60); 
			v[i].tiempo_final.segundos = v[i].tiempo_final.segundos % 60; 
			
			cout << "\n " << (v[i].tiempo_final.segundos);  
			cout << "\n " << v[i].tiempo_final.minutos;
		}
	}
}
int main() {
	...
	int pos = 0;
	int minutoss = 999999;
	int segundoss = 999999;
	for(size_t i=0;i<v.size();i++) { 
		if(v[i].tiempo_final.minutos < minutoss or (v[i].tiempo_final.minutos == minutoss && v[i].tiempo_final.segundos < segundoss)) {
			minutoss = v[i].tiempo_final.minutos;
			segundoss = v[i].tiempo_final.segundos;
			pos = i;
			ganador = v[i].nombre;
		}
		
	}
	cout << "El Ganador es: " << ganador << " con un tiempo de: " << minutoss << "min y " << segundoss << "s";
	return 0;
}
